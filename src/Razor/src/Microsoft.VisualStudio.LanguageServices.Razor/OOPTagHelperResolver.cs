...
namespace Microsoft.VisualStudio.LanguageServices.Razor
{
    internal class OOPTagHelperResolver : TagHelperResolver
    {
        ...
        public override async Task<TagHelperResolutionResult> GetTagHelpersAsync(Project project, ProjectSnapshot projectSnapshot, CancellationToken cancellationToken = default)
        {
            if (project == null)
            {
            ...
            try
            {
                ...
                if (factory != null)
                {
                    result = await ResolveTagHelpersOutOfProcessAsync(factory, project, projectSnapshot).ConfigureAwait(false);
                }

                if (result == null)
                {
                    // Was unable to get tag helpers OOP, fallback to default behavior.
                    result = await ResolveTagHelpersInProcessAsync(project, projectSnapshot).ConfigureAwait(false);
                }
        ...
        protected virtual async Task<TagHelperResolutionResult> ResolveTagHelpersOutOfProcessAsync(IProjectEngineFactory factory, Project project, ProjectSnapshot projectSnapshot)
        {
            ...
            try
            {
                ...
                if (client != null)
                {
                    using (var session = await client.CreateSessionAsync(project.Solution).ConfigureAwait(false))
                    {
                        if (session != null)
                        {
                            var args = new object[]
